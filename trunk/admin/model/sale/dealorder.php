<?php
class ModelSaleDealorder extends Model {
	
    public function editOrder($order_id, $data) {
        $this->db->query("UPDATE `order` SET status = '" . $this->db->escape($data['order_status']) . "' WHERE order_id = '" . (int)$order_id . "' ");
	...
	public function getOrder($order_id) {
	   $sql = "SELECT o.order_id, o.date_added, o.email, o.total, o.telephone, o.date_added, o.product_id,
	...
	public function getOrders($data = array()) {
		$sql = "SELECT o.order_id, o.date_added, o.total, 
                      ...
                      (SELECT NAME FROM product_description WHERE language_id = 2 AND product_id = o.product_id) AS deal_name
                FROM `order` o";
		...
		if (!empty($data['filter_tripname'])) {
			$sql .= " AND deal_name LIKE '" . (float)$data['filter_tripname'] . "%'";
		}
		$sort_data = array(
			'o.order_id',
			'customer',
    ...
    public function getTotalOrders($data = array()) {
		...
		if (!empty($data['filter_tripname'])) {
			$sql .= " AND product_id IN (SELECT product_id FROM product_description pd WHERE pd.language_id = 2 AND pd.name LIKE '" . $data['filter_tripname'] . "%'";
		}
	...
	public function getRelatedDeal($product_id) {
		$sql = "SELECT p.product_id, p.image, 
                	(SELECT NAME FROM product_description WHERE product_id = p.product_id AND language_id = 1) AS productname, 
                                	p.date_added, p.price, p.status, p.productType
                FROM product p 
