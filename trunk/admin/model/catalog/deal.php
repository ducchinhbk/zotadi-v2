<?php
class ModelCatalogDeal extends Model {
	public function addProduct($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "product SET partner_id = '" . $this->db->escape($data['partner_id']) . "', date_available = '" . $this->db->escape($data['date_available']) . "', price = '" . (float)$data['price'] . "', dis_price = '" . (float)$data['dis_price'] . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW()");
	...
	public function editProduct($product_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "product SET   date_available = '" . $this->db->escape($data['date_available']) . "', partner_id = '" . (int)$data['partner_id'] . "', price = '" . (float)$data['price'] . "', dis_price = '" . (float)$data['dis_price'] . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "', date_modified = NOW() WHERE product_id = '" . (int)$product_id . "'");
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT keyword FROM url_alias WHERE query = 'product_id=" . (int)$product_id . "') AS keyword 
	...
	public function getProducts($data = array()) {
		$sql = "SELECT p.product_id, pd.name, p.image, 
                       (SELECT vi_name FROM partner WHERE partner_id = p.partner_id) AS partner,
                       p.price, p.dis_price, p.date_added, p.partner_id, p.status,
                       (SELECT status_name FROM product_status WHERE status_id = p.status) AS `status_name`
                FROM product p LEFT JOIN product_description pd ON p.product_id = pd.product_id
		...
		if (!empty($data['filter_partner'])) {
			$sql .= " AND p.partner_id IN (SELECT partner_id FROM partner WHERE vi_name LIKE '" . $this->db->escape($data['filter_partner']) . "%')";
		}
