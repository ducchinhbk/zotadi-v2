<?php
class ModelSaleItiorder extends Model {
	
    public function editOrder($order_id) {
        $order_partners = $this->request->post['order_parter'];
        
		if (isset($order_partners)) {
            $this->db->query("DELETE FROM tour_custom_order_partner WHERE order_id = '" . (int)$order_id . "'");
			
            foreach ($order_partners as $partner_id) {
				$this->db->query("INSERT INTO tour_custom_order_partner SET partner_id = '" . (int)$partner_id . "', order_id = '" . (int)$order_id . "'");
			}		
		}
    }
    public function deleteOrder($order_id, $product_id) {
        $this->db->query("DELETE FROM tour_custom_order_partner WHERE order_id = '" . (int)$order_id . "'");
        $this->db->query("DELETE FROM tour_custom_orders WHERE custom_tour_order_id = '" . (int)$order_id . "'");
    ...
    public function getProductIdsByOrderID($order_id) {
		...
		$query = $this->db->query("SELECT product_id FROM product WHERE tour_custom_order_id = '" . (int)$order_id . "'");
	...
	public function getOrder($order_id) {
	   $sql = "SELECT o.custom_tour_order_id, 
	               ...
	               o.tripname, o.startdate, o.numdate, o.continent, o.countries, o.cities, o.travelStyle, o.accommodation, 
	               o.includeLike, o.minBudget, o.maxBudget, o.residence, o.numAdults,o.numChild, o.numUnderChild, o.customer_id
	...
	public function getOrders($data = array()) {
		...
		if (!empty($data['filter_tripname'])) {
			$sql .= " AND o.tripname LIKE '" . (float)$data['filter_tripname'] . "%'";
		}
		$sort_data = array(
    ...
    public function getTotalOrders($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM tour_custom_orders";
		...
		if (!empty($data['filter_tripname'])) {
			$sql .= " AND tripname LIKE '" . $data['filter_tripname'] . "%'";
		}
	...
	public function getRelatedItinerary($order_id) {
		$sql = "SELECT p.product_id, p.image, p.price, p.status, p.productType,
                	(SELECT NAME FROM product_description WHERE product_id = p.product_id AND language_id = 1) AS productname, 
                         (SELECT vi_name FROM partner WHERE partner_id = p.partner_id) AS partner_name      	
                FROM product p 
                WHERE p.tour_custom_order_id = '" . (int)$order_id . "' AND p.productType =2;";
	...
	public function getOrderPartner($order_id){
        $query = $this->db->query("SELECT partner_id FROM tour_custom_order_partner WHERE order_id = '" . (int)$order_id . "'");
