...
class ControllerCatalogItinerary extends Controller
{
    ...
    public function insert()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_catalog_product->addItinerary($this->request->post, $partner_id, $order_id);
    ...
    protected function getList()
    {
        ...
        $product_total = $this->model_catalog_product->getTotalProducts($data, $partner_id);

        $results = $this->model_catalog_product->getProducts($data, $partner_id);
        
        //get all status of itineray
        $filter_status = $this->model_catalog_product->getProductStatus();
        ...
        foreach ($results as $result) {
            ...
            $action[] = array(
                ...
                'href' => $this->url->link('catalog/itinerary/update', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'].'&order_id='. $result['tour_custom_order_id'] . $url, 'SSL')
            );
            ...
            $this->data['products'][] = array(
                ...
                'name' => $result['name'],
                'order_id' => $result['tour_custom_order_id'],
                'price' => $result['price'],
                'special' => $special,
                'image' => $image,
        ...
        $this->data['sort_order_id'] = $this->url->link('catalog/itinerary', 'token=' . $this->session->data['token'] . '&sort=p.tour_custom_order_id' . $url, 'SSL');
        $this->data['sort_price'] = $this->url->link('catalog/itinerary', 'token=' . $this->session->data['token'] . '&sort=p.price' . $url, 'SSL');
        $this->data['sort_quantity'] = $this->url->link('catalog/itinerary', 'token=' . $this->session->data['token'] . '&sort=p.quantity' . $url, 'SSL');
        $this->data['sort_status'] = $this->url->link('catalog/itinerary', 'token=' . $this->session->data['token'] . '&sort=p.status' . $url, 'SSL');
        $this->data['sort_order'] = $this->url->link('catalog/itinerary', 'token=' . $this->session->data['token'] . '&sort=p.sort_order' . $url, 'SSL');
    ...
    protected function getForm($order_id)
    {
        $this->data['heading_title'] = $this->language->get('heading_title');
        ...
        if (!isset($this->request->get['product_id'])) {
            $this->data['action'] = $this->url->link('catalog/itinerary/insert', 'token=' . $this->session->data['token'] . '&order_id=' .$order_id .$url, 'SSL');
        } else {
            $this->data['action'] = $this->url->link('catalog/itinerary/update', 'token=' . $this->session->data['token'] . '&product_id=' . $this->request->get['product_id'] . $url, 'SSL');
        }
